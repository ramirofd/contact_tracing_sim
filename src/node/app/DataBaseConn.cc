...
void DataBaseConn::createWindowTable(){
    // Create a table
    const char *createTableSQL = "CREATE TABLE IF NOT EXISTS window (id INTEGER PRIMARY KEY AUTOINCREMENT, node INTEGER, contact_id INTEGER, told_by_id INTEGER, hop INTEGER, start REAL, end REAL, closed INTEGER, run_id INTEGER, velocity REAL, FOREIGN KEY(run_id) REFERENCES simulation(run))";
    int rc = sqlite3_exec(this->myDB, createTableSQL, 0, 0, 0);
    if (rc != SQLITE_OK) {
        EV_ERROR << "Failed to execute statement: " << sqlite3_errmsg(this->myDB);
        sqlite3_close(this->myDB);
...
void DataBaseConn::insertWindow(int node, ContactWindow* win, double velocity){
    // Prepare an INSERT statement
    const char *insertSQL = "INSERT INTO window (node, contact_id, told_by_id, hop, start, end, closed, run_id, velocity) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)";
    ...
    sqlite3_bind_double(insertStmt, 9, velocity);
